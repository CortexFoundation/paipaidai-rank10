...
if __name__ == '__main__':
    ...
    model = gensim.models.KeyedVectors.load_word2vec_format('data/word_embed_vec.txt', binary=False)

    X_train=build_features(df_train)
    X_test = build_features(df_test)
    print(X_train)
    path = "feature/"
    X_train.to_csv('feature/feature_cnn2_train.csv', index=False)
    X_test.to_csv('feature/feature_cnn2_test.csv', index=False)
