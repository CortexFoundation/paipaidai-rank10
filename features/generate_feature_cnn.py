...
def build_features(data):
    ...
    X['len_q1_word'] = data.words_x.apply(lambda x: len(x))  # 1
    X['len_q2_word'] = data.words_y.apply(lambda x: len(x))  # 1
    X['len_q1_char'] = data.chars_x.apply(lambda x: len(x))  # 1
    X['len_q2_char'] = data.chars_y.apply(lambda x: len(x))  # 1
...
if __name__ == '__main__':
    ...
    X_train.to_csv("feature/feature_cnn_trian.csv", index=False)
    X_test.to_csv("feature/feature_cnn_test.csv", index=False)
