...
for idx_train, idx_val in skf:
    ...
    model = cnn_lstm(word_embedding_matrix,leaks.shape[1],nb_words)
    early_stop = EarlyStopping(patience=2)
    check_point = ModelCheckpoint('paipaidai.hdf5', monitor="val_loss", mode="min", save_best_only=True, verbose=1)
    history = model.fit([X_train_1_tr,X_train_2_tr,leaks_tr], y_tr, batch_size = 1024, epochs = 100,validation_data=([X_train_1_te,X_train_2_te,leaks_te], y_te),callbacks=[early_stop,check_point])
...
print(score)
name="cnn_lstm_%s"%str(round(score,6))
print(score)
t_p = pd.DataFrame()
t_p[name]=te_pred
