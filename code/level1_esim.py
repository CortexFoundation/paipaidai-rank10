...
X_train_1 = pad_sequences(sequences_all, maxlen=MAX_SEQUENCE_LENGTH)
X_test_1 = pad_sequences(sequences_test, maxlen=MAX_SEQUENCE_LENGTH)
column="w2"
sequences_all = tokenizer.texts_to_sequences(list(train[column]))
sequences_test = tokenizer.texts_to_sequences(list(test[column]))
X_train_2 = pad_sequences(sequences_all, maxlen=MAX_SEQUENCE_LENGTH)
X_test_2 = pad_sequences(sequences_test, maxlen=MAX_SEQUENCE_LENGTH)
...
for idx_train, idx_val in skf:
    ...
    history = model.fit([X_train_1_tr,X_train_2_tr,leaks_tr], y_tr, batch_size = 1024, epochs = 3,validation_data=([X_train_1_te,X_train_2_te,leaks_te], y_te),callbacks=[early_stop,check_point])
...
print(score)
name="esim_%s"%str(round(score,6))
print(score)
t_p = pd.DataFrame()
t_p[name]=te_pred
