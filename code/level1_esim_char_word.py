...
tokenizer.fit_on_texts(list(train["c1"])+list(test["c1"])+list(train["w2"])+list(test["w2"]))
column="c1"
...
for idx_train, idx_val in skf:
    ...
    model = esim(word_embedding_matrix,leaks.shape[1],maxlen=MAX_SEQUENCE_LENGTH,lstm_dim=300,dense_dropout=0.2)
...
print(score)
name="esim_char_word_%s"%str(round(score,6))
print(score)
t_p = pd.DataFrame()
t_p[name]=te_pred
